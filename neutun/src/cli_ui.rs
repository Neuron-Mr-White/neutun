    ...
    fn get_sub_domain_notice(&self, sub_domain: &str) -> Option<String> {
            ...
            && (self.config.sub_domain.as_ref().map(|s| s.as_str()) != Some(sub_domain))
        {
            if self.config.secret_key.is_some() {
                Some(format!("{}",
                          "To use custom sub-domains feature, please upgrade your billing plan at https://dashboard.neutun.dev.".yellow()))
            } else {
                Some(format!("{}",
                          "To access the sub-domain feature, get your authentication key at https://dashboard.neutun.dev.".yellow()))
            }
        } else {
            None
        }
